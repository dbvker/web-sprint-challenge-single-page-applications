...
const Order = () => {
  ...
  useEffect(() => {
    formSchema.isValid(pizzaValues).then((valid) => setDisabled(!valid));
  }, [pizzaValues]);
  ...
  return (
    <>
      ...
      {/* {pizza && <h3><center>Orders</center></h3>}
        {pizza.map(order => {
          return (
            <div className="order-box">
              ...
              <p>Toppings:
                <ul>
                  <li>Pepperoni: {order.pepperoni ? 'Yes' : 'No'}</li>
                  <li>Sausage: {order.sausage ? 'Yes' : 'No'}</li>
                  <li>Diced Tomatoes: {order.tomatoes ? 'Yes' : 'No'}</li>
                  <li>Black Olives: {order.olives ? 'Yes' : 'No'}</li>
                </ul>
              </p>
              <p>Gluten Free: {order.glutenFree ? 'Yes' : 'No'}</p>
              <p>Special Instructions: {order.special ? order.special : 'N/A'}</p>
            </div>
          )
        })}
      } */}
